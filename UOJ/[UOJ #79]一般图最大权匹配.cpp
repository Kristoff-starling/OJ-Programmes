...
int mat[MAXN+48],visited[MAXN+48],pre[MAXN+48],mark[MAXN+48];
...
inline int bfs(int st)
{
    ...
    while (head<=tail)
    {
        ...
        for (auto y : v[cur])
        {
            if (find_anc(y)==find_anc(cur)) continue;
            if (mark[y]==-1)
            {
                mark[y]=1;pre[y]=cur;
...
int main ()
{
    ...
    for (register int i=1;i<=m;i++) scanf("%d%d",&x,&y),v[x].pb(y),v[y].pb(x);
    for (register int i=1;i<=n;i++) mat[i]=-1;
    for (register int i=1;i<=n;i++) if (mat[i]==-1) ans+=bfs(i);
    printf("%d\n",ans);for (register int i=1;i<=n;i++) if (mat[i]!=-1) printf("%d ",mat[i]); else printf("0 ");puts("");
    io.flush();
#ifdef LOCAL
