...
int n,q,rt,val[MAXN+48];
int tot,in[MAXN+48],out[MAXN+48],depth[MAXN+48],sz[MAXN+48],top[MAXN+48],son[MAXN+48],fa[MAXN+48];LL ds[MAXN+48];vector<int> v[MAXN+48];

struct BIT
{
	LL c[MAXN+48];
	inline void init() {rep(i,1,n) c[i]=0;}
	inline void modify(int x,LL delta) {while (x<=n) c[x]+=delta,x+=LOWBIT(x);}
	inline LL query(int x) {LL res=0;while (x) res+=c[x],x^=LOWBIT(x);return res;}
	inline LL calc(int l,int r) {return query(r)-query(l-1);}
}sum,tsum,dsum;
...
inline void dfs1(int cur,int father)
{
	...
	for (auto y : v[cur])
		if (y!=father)
		{
			depth[y]=depth[cur]+1;ds[y]=ds[cur]+val[y];dfs1(y,cur);
			sz[cur]+=sz[y];if (!son[cur] || sz[son[cur]]<sz[y]) son[cur]=y;
		}
}

inline void dfs2(int cur,int tp)
{
	in[cur]=++tot;top[cur]=tp;
...
inline int query_lca(int x,int y)
{
	...
	while (tp1!=tp2)
	{
		if (depth[tp1]<depth[tp2]) swap(tp1,tp2),swap(x,y);
		x=fa[tp1];tp1=top[x];
	}
	if (depth[x]>depth[y]) swap(x,y);return x;
}

inline LL calc(int x)
{
	if (!x) return 0;
	return tsum.query(in[x])*(depth[x]+1)-dsum.query(in[x]);
}
...
int main ()
{
	...
	cerr<<"Running..."<<endl;
#endif
	io.Get(n,q,rt);rep(i,1,n) io.Get(val[i]);
	int op,x,y,c,LCA;rep(i,1,n-1) io.Get(x,y),v[x].pb(y),v[y].pb(x);
	depth[rt]=1;ds[rt]=val[rt];dfs1(rt,-1);dfs2(rt,rt);
	while (q--)
	{
		io.Get(op);
		if (op==1) io.Get(x,c),sum.modify(in[x],c),sum.modify(out[x]+1,-c);
		if (op==2) io.Get(x,c),tsum.modify(in[x],c),tsum.modify(out[x]+1,-c),dsum.modify(in[x],1ll*c*depth[x]),dsum.modify(out[x]+1,1ll*(-c)*depth[x]);
		if (op==3)
		{
			io.Get(x,y);
			LCA=query_lca(x,y);LL ans=calc(x)+calc(y)-calc(LCA)-calc(fa[LCA]);
			ans+=sum.query(in[x])+sum.query(in[y])-sum.query(in[LCA])-sum.query(in[fa[LCA]]);
			ans+=ds[x]+ds[y]-ds[LCA]-ds[fa[LCA]];
			printf("%lld\n",ans);
		}
	}
	io.flush();
