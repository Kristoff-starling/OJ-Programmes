...
{
	rep(i,1,n) dist[i]=LINF;dist[s]=0;q.push(mp(0,s));
	while (!q.empty())
	{
		LL dd=-q.top().x,x=q.top().y;q.pop();
		if (dd>dist[x]) continue;
		for (auto item : v[x])
			if (dist[x]+item.y<dist[item.x])
			{
				dist[item.x]=dist[x]+item.y;
...
int main ()
{
	...
	rep(i,1,m) Get(x,y,c),v[x].pb(mp(y,c)),v[y].pb(mp(x,c));
	dijkstra();printf("%lld\n",dist[t]);
	io.flush();
#ifndef ONLINE_JUDGE
