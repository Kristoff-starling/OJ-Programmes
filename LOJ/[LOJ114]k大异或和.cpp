...
{
	LL b[68];int cnt;int ze;
	inline void init() {memset(b,0,sizeof(b));cnt=0;ze=1;}
	inline void insert(LL x)
	{
		REP(i,52,0) if (x>>i&1)
			if (b[i]) x^=b[i]; else {b[i]=x;cnt++;return;}
		if (x==0) ze=0;
	}
	inline void pre()
	{
		REP(i,52,0) if (b[i])
			REP(j,i-1,0) if ((b[i]>>j&1) && b[j]) b[i]^=b[j];
	}
	inline LL query(LL k)
	{
		if (pw[cnt]-ze<k) return -1;
		LL res=0;int c=cnt;
		REP(i,52,0)
		{
			if (!b[i]) continue;
			c--;if (pw[c]-ze<k) res^=b[i],k-=pw[c];
		}
		return res;
	}
}
...
int main ()
{
	...
	cerr<<"Running..."<<endl;
#endif
	Get(n);LL x;LinearBase::init();
	rep(i,1,n) Get(x),LinearBase::insert(x);
	pw[0]=1;rep(i,1,52) pw[i]=pw[i-1]+pw[i-1];LinearBase::pre();
	Get(q);while (q--) Get(x),printf("%lld\n",LinearBase::query(x));
	io.flush();
#ifndef ONLINE_JUDGE
