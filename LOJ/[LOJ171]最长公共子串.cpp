...
namespace SAM
{
	int nxt[MAXN*2][27],par[MAXN*2],maxn[MAXN*2],maxlen[MAXN*2][15],last,root,tot;
	inline int Create() {++tot;par[tot]=maxn[tot]=0;memset(nxt[tot],0,sizeof(nxt[tot]));memset(maxlen[tot],0,sizeof(maxlen[tot]));return tot;}
	inline void init() {tot=0;root=last=Create();}
	inline void extend(char ch)
	{
		int p=last,np=Create(),w=ch-'a'+1;maxn[np]=maxn[p]+1;
	...
	inline void go(char s[],int ind)
	{
		int cur=root,len=0,l=strlen(s+1);
		rep(i,1,l)
		{
			int w=s[i]-'a'+1;
			while (cur && !nxt[cur][w]) cur=par[cur],len=maxn[cur];
			if (!cur) cur=root,len=0; else cur=nxt[cur][w],len++;
			check_max(maxlen[cur][ind],len);
		}
	}
	int ord[MAXN*2];
	...
	inline void pushup()
	{
		rep(i,1,tot) ord[i]=i;sort(ord+1,ord+tot+1,cmp);
		rep(id,1,tot)
		{
			int cur=ord[id],fa=par[cur];
			rep(i,2,n) check_max(maxlen[fa][i],maxlen[cur][i]);
		}
	}
	inline int query()
	{
		...
		{
			int res=maxn[cur];
			rep(i,2,n) check_min(res,maxlen[cur][i]);
			check_max(ans,res);
		}
		return ans;
	}
...
int main ()
{
	...
	cerr<<"Running..."<<endl;
#endif
	scanf("%d",&n);scanf("%s",s+1);SAM::build(s);
	rep(i,2,n) scanf("%s",s+1),SAM::go(s,i);
	SAM::pushup();printf("%d\n",SAM::query());
	io.flush();
#ifndef ONLINE_JUDGE
