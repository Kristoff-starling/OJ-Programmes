...
int n,a[MAXN+48],fac[MAXN+48],ifac[MAXN+48],inv[MAXN+48];
...
int main ()
{
	...
	fac[0]=1;rep(i,1,n) fac[i]=1ll*fac[i-1]*a[i]%MOD;
	ifac[n]=quick_pow(fac[n],MOD-2);REP(i,n-1,1) ifac[i]=1ll*ifac[i+1]*a[i+1]%MOD;
	rep(i,1,n) inv[i]=1ll*fac[i-1]*ifac[i]%MOD;
	int pw=1,res=0;REP(i,n,1) Add(res,int(1ll*pw*inv[i]%MOD)),pw=1ll*pw*998244353%MOD;
	printf("%d\n",res);
	io.flush();
#ifndef ONLINE_JUDGE
