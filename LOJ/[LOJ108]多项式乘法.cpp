...
{
	const int NTT_MAX=8e5,G=3;
	int pw_pos[NTT_MAX+48],pw_neg[NTT_MAX+48];bool isfirst=true;
	inline void poly_init()
	{
		for (register int clen=2;clen<=524288;clen<<=1)
		{
			pw_pos[clen]=quick_pow(G,(MOD-1)/clen);
			pw_neg[clen]=quick_pow(G,(MOD-1)-(MOD-1)/clen);
		}
	}
	inline void poly_ntt(int c[],int len,int fl)
	{
		...
		{
			int wn=(fl==1?pw_pos[clen]:pw_neg[clen]);
		...
		if (fl==-1)
		{
			...
			rep(i,0,len-1) c[i]=1ll*c[i]*ilen%MOD;
		}
	}
	inline void poly_mul(int A[],int B[],int N,int M)
	{
		int len=1;while (len<=N+M) len<<=1;
		memset(A+N,0,(len-N)<<2);memset(B+M,0,(len-M)<<2);
...
#endif
	Get(n,m);n++;m++;
	rep(i,0,n-1) Get(a[i]);rep(i,0,m-1) Get(b[i]);
	polynomial::poly_mul(a,b,n,m);
	rep(i,0,n+m-2) printf("%d ",a[i]);puts("");
	io.flush();
#ifndef ONLINE_JUDGE
