...
inline bool check()
{
	sort(a+1,a+n+1,cmp);double res=0;
	rep(i,1,k) res+=a[i].a-a[i].b*mid;
...
int main ()
{
	...
	cerr<<"Running..."<<endl;
#endif
	Get(n,k);rep(i,1,n) Get(a[i].a);rep(i,1,n) Get(a[i].b);
	double l=INF,r=-1;rep(i,1,n) check_min(l,a[i].a/a[i].b),check_max(r,a[i].a/a[i].b);
	while (r-l>=1e-6)
	{
		mid=(l+r)/2;
