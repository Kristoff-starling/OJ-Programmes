...
{
	LL res=0;
	res+=1ll*(x+1)*(y+1)*bit1.query(x,y);
	res-=1ll*(x+1)*bit2.query(x,y);
	res-=1ll*(y+1)*bit3.query(x,y);
	res+=bit4.query(x,y);
	return res;
}
...
int main ()
{
	...
	cerr<<"Running..."<<endl;
#endif
	scanf("%d%d",&n,&m);int op,x1,y1,x2,y2,c;
	while (scanf("%d%d%d%d%d",&op,&x1,&y1,&x2,&y2)!=EOF)
	{
		if (op==1)
		{
			scanf("%d",&c);
			bit1.modify(x1,y1,c);bit1.modify(x2+1,y1,-c);bit1.modify(x1,y2+1,-c);bit1.modify(x2+1,y2+1,c);
			bit2.modify(x1,y1,1ll*c*y1);bit2.modify(x2+1,y1,1ll*(-c)*y1);bit2.modify(x1,y2+1,1ll*(-c)*(y2+1));bit2.modify(x2+1,y2+1,1ll*c*(y2+1));
			bit3.modify(x1,y1,1ll*c*x1);bit3.modify(x2+1,y1,1ll*(-c)*(x2+1));bit3.modify(x1,y2+1,1ll*(-c)*x1);bit3.modify(x2+1,y2+1,1ll*c*(x2+1));
			bit4.modify(x1,y1,1ll*c*x1*y1);bit4.modify(x2+1,y1,1ll*(-c)*(x2+1)*y1);bit4.modify(x1,y2+1,1ll*(-c)*x1*(y2+1));bit4.modify(x2+1,y2+1,1ll*c*(x2+1)*(y2+1));
		}
		else
		{
