...
#endif
	Get(x,n);
	small[0]=1;rep(i,1,S) small[i]=1ll*small[i-1]*x%MOD;
	big[0]=1;rep(i,1,S) big[i]=1ll*big[i-1]*small[S]%MOD;
	while (n--)
	{
		Get(a);int p1=a/S,p2=a%S;
		printf("%d ",1ll*big[p1]*small[p2]%MOD);
	}
	io.flush();
#ifndef ONLINE_JUDGE
