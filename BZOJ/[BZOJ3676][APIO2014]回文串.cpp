...
namespace PAM
{
	int nxt[MAXN+48][27],len[MAXN+48],fail[MAXN+48],cnt[MAXN+48],tot,last;
	inline int Create() {++tot;memset(nxt[tot],0,sizeof(nxt[tot]));len[tot]=fail[tot]=cnt[tot]=0;return tot;}
	inline void init()
	{
		len[0]=0;len[1]=-1;fail[0]=1;fail[1]=0;cnt[0]=cnt[1]=0;
	...
	inline void extend(int pos)
	{
		...
		last=nxt[fa][w];cnt[last]++;
	}
	inline void build()
	{
		int L=strlen(s+1);init();
		rep(i,1,L) extend(i);
		REP(i,tot,2) cnt[fail[i]]+=cnt[i];
	}
}
...
int main ()
{
	...
	cerr<<"Running..."<<endl;
#endif
	scanf("%s",s+1);PAM::build();
	LL ans=0;
	rep(i,2,PAM::tot) check_max(ans,1ll*PAM::len[i]*PAM::cnt[i]);
	printf("%lld\n",ans);
	io.flush();
#ifndef ONLINE_JUDGE
