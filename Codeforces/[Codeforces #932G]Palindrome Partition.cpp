...
char s[MAXN+48],ss[MAXN+48];int n;
int dp[MAXN+48];

namespace PAM
{
	int nxt[MAXN+48][27],fail[MAXN+48],slink[MAXN+48],len[MAXN+48],diff[MAXN+48],serie[MAXN+48],last,tot;
	inline int Create() {++tot;memset(nxt[tot],0,sizeof(nxt[tot]));fail[tot]=slink[tot]=len[tot]=0;return tot;}
	inline void init()
	{
		len[0]=0;len[1]=-1;fail[0]=1;fail[1]=0;slink[0]=slink[1]=0;diff[0]=1;diff[1]=-1;
		memset(nxt[0],0,sizeof(nxt[0]));memset(nxt[1],0,sizeof(nxt[1]));
		tot=last=1;
	}
	inline int getfail(int pos,int x) {while (s[pos]!=s[pos-len[x]-1]) x=fail[x];return x;}
	inline void extend(int pos)
	{
		...
		if (!nxt[fa][w])
		{
			...
			if (diff[cur]==diff[fail[cur]]) slink[cur]=slink[fail[cur]]; else slink[cur]=fail[cur];
			nxt[fa][w]=cur;
		}
		last=nxt[fa][w];
	}
...
int main ()
{
	...
	for (register int i=1,j=n,pt=1;i<j;i++,j--,pt+=2) ss[pt]=s[i],ss[pt+1]=s[j];
	rep(i,1,n) s[i]=ss[i];
	PAM::build();printf("%d\n",dp[n]);
	io.flush();
#ifndef ONLINE_JUDGE
