...
int n,m,f[MAXN+48][MAXN+48],g[MAXN+48][MAXN+48];
...
int main ()
{
	...
	cerr<<"Running..."<<endl;
#endif
	Get(n,m);init_dp();int ans=0,sum=0;
	rep(i,1,n)
	{
		sum=0;
		rep(x,i,n)
		{
			int y=x-i;if (y<0) continue;int tmp=f[x][y];
			int lx=n-x,ly=m-y;if (lx<0 || ly<0) continue;tmp=1ll*tmp*g[lx][ly]%MOD;
			Add(sum,tmp);
		}
		Add(ans,int(1ll*sum*i%MOD));
	}
	printf("%d\n",ans);
#ifndef ONLINE_JUDGE
