...
char s[MAXN+48];int sum1[MAXN+48],sum2[MAXN+48],delta[MAXN+48],maxn[MAXN+48];
int ans[MAXN+48];
...
int main ()
{
	...
	sum1[0]=sum2[0]=0;rep(i,1,len) sum1[i]=sum1[i-1]+(s[i]=='0'),sum2[i]=sum2[i-1]+(s[i]=='1'),delta[i]=sum1[i]-sum2[i];
	maxn[len+1]=0;maxn[len]=delta[len];REP(i,len-1,1) maxn[i]=max(maxn[i+1],delta[i]);
	int base=0;
	rep(i,1,len)
	{
		if (s[i]=='0') {ans[i]=0;base++;continue;}
		int pt=i;while (pt<=len && s[pt]=='1') pt++;pt--;
		base-=pt-i+1;
		int mx=max(pt-i+1-(maxn[pt+1]-((pt+1)>len?0:base)),0);
		rep(j,i,i+mx-1) ans[j]=0;rep(j,i+mx,pt) ans[j]=1;
		i=pt;
	}
