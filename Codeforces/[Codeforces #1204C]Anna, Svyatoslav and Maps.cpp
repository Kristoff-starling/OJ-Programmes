...
int n,m,ga[148][148];
int seq[1000048],ans[1000048],tot;
...
int main ()
{
	...
	{
		scanf("%s",s+1);
		rep(j,1,n) if (s[j]=='1') ga[i][j]=1; else ga[i][j]=INF;
		ga[i][i]=0;
	}
	floyd();
	Get(m);rep(i,1,m) Get(seq[i]);
	tot=1;ans[1]=seq[1];int pt=2,lpt=1;bool last=false;
	for (;;)
	{
		while (pt<=m && ga[ans[tot]][seq[pt]]==pt-lpt) pt++;
		if (pt>m) break;
		pt--;ans[++tot]=seq[pt];if (pt==m) last=true;lpt=pt;pt++;
	}
	if (!last) ans[++tot]=seq[m];
	printf("%d\n",tot);rep(i,1,tot) printf("%d ",ans[i]);
	puts("");
#ifndef ONLINE_JUDGE
