    ...
    void poly_ntt(int c[],int len,int fl)
    {
        ...
        for (clen=2;clen<=len;clen<<=1)
        {
            ...
            for (j=0;j<len;j+=clen)
            {
                ...
                {
                    tmp1=c[k];tmp2=1ll*(*w)*c[k+(clen>>1)]%MOD;
...
void solve(int left,int right)
{
    if (left==right)
    {
        ...
        if (ch1!='W' && ch2!='W') tmp[0]=1; else tmp[0]=0;
        if (ch1!='B' && ch2!='B') tmp[2]=1; else tmp[2]=0;
        if (ch1=='?' && ch2=='?') tmp[1]=2;
        else if (ch1==ch2) tmp[1]=0;
        else tmp[1]=1;
        return;
    }
    int mid=(left+right)>>1,N=(mid-left+1)*2+1,M=(right-mid)*2+1;
    int len=1;while (len<=N+M) len<<=1;
    int Tmp[len+48];
