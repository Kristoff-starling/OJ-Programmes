...
int n,m;
int a[348];char p[548][10],s[548][10];int Wp[548],Ws[548],ptot,stot;

namespace flow
{
	int head[100048],cur[100048],to[200048],nxt[200048],f[200048],tot,t;
	inline void init(int _t) {t=_t;rep(i,0,t) head[i]=0;tot=1;}
...
namespace Trie
{
	int nxt[100048][2],lim[100048],toind[100048],ind[100048],tot,Root1,Root2;
	inline int Create() {++tot;memset(nxt[tot],0,sizeof(nxt[tot]));lim[tot]=INF;toind[tot]=ind[tot]=0;return tot;}
	inline void init() {tot=-1;Root1=Create();Root2=Create();}
	inline void insert1(int val,int id)
	{
		int cur=Root1;
		rep(i,1,8)
		{
	...
	inline void insert2(int val,int id)
	{
		int cur=Root2;
		rep(i,1,8)
		{
	...
	inline void insert_p(char s[],int w)
	{
		int len=strlen(s+1),cur=Root1;
		rep(i,1,len)
		{
			int w=s[i]-'0';
			if (!nxt[cur][w]) return;
	...
	inline void insert_s(char s[],int w)
	{
		int len=strlen(s+1),cur=Root2;
		REP(i,len,1)
		{
			int w=s[i]-'0';
			if (!nxt[cur][w]) return;
	...
	{
		flow::addedge(cur,toind[cur],lim[cur]);
		if (ind[cur]) flow::addedge(toind[cur],tot+ind[cur],INF);
		if (nxt[cur][0]) flow::addedge(toind[cur],nxt[cur][0],INF),dfs1(nxt[cur][0]);
		if (nxt[cur][1]) flow::addedge(toind[cur],nxt[cur][1],INF),dfs1(nxt[cur][1]);
	}
	inline void dfs2(int cur)
	{
		flow::addedge(toind[cur],cur,lim[cur]);
		if (ind[cur]) flow::addedge(tot+ind[cur],toind[cur],INF);
		if (nxt[cur][0]) flow::addedge(nxt[cur][0],toind[cur],INF),dfs2(nxt[cur][0]);
		if (nxt[cur][1]) flow::addedge(nxt[cur][1],toind[cur],INF),dfs2(nxt[cur][1]);
	}
	inline void build()
	{
		int tmp=tot;rep(i,0,tmp) toind[i]=++tot;
		flow::init(tot+n);
...
inline void input()
{
	...
	rep(i,1,m)
	{
		...
		{
			ptot++;
			scanf("%s%d",p[ptot]+1,&Wp[ptot]);
		}
		else
		{
			stot++;
			scanf("%s%d",s[stot]+1,&Ws[stot]);
		}
	}
}
...
inline void solve(int nu)
{
	rep(i,1,n) Trie::insert1(a[i],i);
	rep(i,1,n) Trie::insert2(a[i],i);
	rep(i,1,ptot) Trie::insert_p(p[i],Wp[i]);
	rep(i,1,stot) Trie::insert_s(s[i],Ws[i]);
	Trie::build();int res=flow::dinic();if (res>=INF) res=-1;
	printf("Case #%d: %d\n",nu,res);
}
