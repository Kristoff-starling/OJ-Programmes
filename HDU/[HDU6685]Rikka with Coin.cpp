...
int n,a[1048];
bool exist[48];bool dp[48];int val[48];
...
int main ()
{
	...
	while (ca--)
	{
		Get(n);rep(i,1,n) Get(a[i]);
		sort(a+1,a+n+1);
		bool f=true;rep(i,1,n) if (a[i]%10) {f=false;break;}
		if (!f) {puts("-1");continue;}
		rep(i,1,9) exist[i]=false;
		LL ans=a[n]/100;
		int minn=10;
		rep(i,0,1) rep(j,0,3) rep(k,0,1)
		{
			memset(dp,false,sizeof(dp));
			dp[0]=true;int tot=0;rep(l,1,i) val[++tot]=1;rep(l,1,j) val[++tot]=2;rep(l,1,k) val[++tot]=5;
			rep(l,1,tot)
			{
				int d=val[l];
				REP(m,20,0) if (dp[m]) dp[m+d]=true;
			}
			f=true;int d=0;
			rep(l,1,n-1)
			{
				if (dp[(a[l]%100)/10]) continue;
				if (a[l]>=100 && dp[(a[l]%100+100)/10]) continue;
				f=false;
			}
			if (a[n]>=100 && dp[(a[n]%100+100)/10]) d=-1;
			else if (dp[a[n]%100/10]) d=0;
			else f=false;
			if (f) check_min(minn,i+j+k+d);
		}
		printf("%lld\n",ans+minn);
	}
	io.flush();
