...
int R,C,n,X[MAXN+48],Y[MAXN+48];char dir[MAXN+48];
int val[MAXN+48],id[MAXN+48];
...
int main ()
{
	...
	while (ca--)
	{
		scanf("%d%d%d",&R,&C,&n);int op[5];rep(i,1,n) scanf("%d%d%s",X+i,Y+i,op+1),dir[i]=op[1];
		rep(i,1,n) val[i]=Y[i];sort(val+1,val+n+1);
		rep(i,1,n) Y[i]=lower_bound(val+1,val+n+1,Y[i])-val;
		up.init();down.init();rght.init();
		rep(i,1,n) id[i]=i;sort(id+1,id+n+1,[&](const int &x,const int &y){return X[x]<X[y];});
		LL ans=0;
		rep(i,1,n)
		{
			if (dir[id[i]]=='U') ans+=rght.calc(Y[id[i]],n),up.modify(Y[id[i]],1);
			if (dir[id[i]]=='D') ans+=rght.calc(1,Y[id[i]]),down.modify(Y[id[i]],1);
			if (dir[id[i]]=='L') ans+=up.calc(1,Y[id[i]])+down.calc(Y[id[i]],n);
			if (dir[id[i]]=='R') rght.modify(Y[id[i]],1);
		}
		printf("%lld\n",ans+1);
	}
	io.flush();
