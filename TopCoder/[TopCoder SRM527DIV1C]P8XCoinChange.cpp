...
struct Matrix
{
	...
	inline void init_I() {rep(i,1,n) b[i][i]=1;}
	...
	{
		Matrix res;
		rep(i,1,n) rep(j,1,n) rep(k,1,n) Add(res.b[i][j],mul(b[i][k],other.b[k][j]));
		return res;
	}
}M[48],ans;
...
{
	public:
		inline int solve(LL coin_sum,vector<LL> values)
		{
			n=int(values.size());rep(i,1,n) val[i]=values[i-1];
			rep(i,1,n) M[i].clear();
			rep(i,1,n) M[1].b[i][1]=1;
			rep(i,2,n)
			{
				M[i]=quick_pow(M[i-1],val[i]/val[i-1]);
				rep(j,i,n) Add(M[i].b[j][i],1);
			}
			ans.clear();ans.init_I();
			REP(i,n,1)
			{
				LL t=coin_sum/val[i];ans=ans*quick_pow(M[i],t);
				coin_sum%=val[i];
			}
			int res=0;
			rep(i,1,n) Add(res,ans.b[n][i]);
			return res;
		}
};
...
int main ()
{
	...
	cerr<<"Running..."<<endl;
#endif
	LL sum;vector<LL> vv;LL x;
	P8XCoinChange A;
	while (cin>>sum)
	{
		int nn;cin>>nn;vv.clear();
		while (nn--) cin>>x,vv.pb(x);
		cout<<A.solve(sum,vv)<<endl;
	}
	io.flush();
#ifndef ONLINE_JUDGE
