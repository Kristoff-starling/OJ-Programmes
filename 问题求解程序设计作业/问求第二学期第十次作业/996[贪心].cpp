...
int main ()
{
    ...
    tot=0;rep(i,1,n) val[++tot]=a[i].x,val[++tot]=a[i].y;
    sort(val+1,val+tot+1);tot=unique(val+1,val+tot+1)-(val+1);
    rep(i,1,n)
    {
        a[i].x=lower_bound(val+1,val+tot+1,a[i].x)-val;a[i].y=lower_bound(val+1,val+tot+1,a[i].y)-val;
        sum[a[i].x]++;sum[a[i].y]--;
    }
    int ans=0;
    rep(i,1,tot) sum[i]+=sum[i-1],check_max(ans,sum[i]);
    printf("%d\n",ans);
#ifndef ONLINE_JUDGE
