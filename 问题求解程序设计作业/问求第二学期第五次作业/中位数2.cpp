...
int main ()
{
    ...
    s1.insert(a[1]);printf("%d\n",a[1]);
    s1.insert(a[2]);s2.insert(*s1.begin());s1.erase(s1.begin());
    for (int i=3;i<=n;i++)
    {
        if (a[i]>=(*--s2.end())) s1.insert(a[i]); else s2.insert(a[i]);
        if (int(s2.size())>int(s1.size()))
            s1.insert(*(--s2.end())),s2.erase(--s2.end());
