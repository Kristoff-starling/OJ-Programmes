...
namespace PAM
{
	int nxt[MAXN+48][27],len[MAXN+48],fail[MAXN+48],tot,last;
	inline int Create() {++tot;memset(nxt[tot],0,sizeof(nxt[tot]));len[tot]=fail[tot]=0;return tot;}
	inline void init()
	{
		tot=-1;Create();Create();
	...
	inline void solve()
	{
		...
		rep(i,1,len) extend(i),printf("%d ",tot-1);
		puts("");
	}
}
